      ...
      <AnimatePresence>
        {isOpen ? (
          <motion.nav
            ...
            className="absolute top-24 right-0 z-25 w-[calc(100%-2rem)] max-w-85.75 bg-arch-very-light-grey py-10 pl-12 ml-3.75 shadow-nav md:hidden"
            initial={
            ...
            transition={
              shouldReduceMotion ? { duration: 0.12 } : mobileNavTransition
            }
            aria-label="Primary navigation">
            <NavigationList isActive={isActive} setIsOpen={setIsOpen} />
          </motion.nav>
        ) : null}
      </AnimatePresence>

      <nav
        className="hidden md:static md:block md:w-auto md:max-w-none md:bg-transparent md:p-0 md:shadow-none"
        aria-label="Primary navigation">
        <NavigationList isActive={isActive} setIsOpen={setIsOpen} />
      </nav>
    </>
...
function NavigationList({
  isActive,
  setIsOpen,
}: Pick<NavLinkProps, "isActive" | "setIsOpen">) {
  return (
